...
## Supports fill, border and shadow. Each is state-config driven.
##
## Border is drawn INSIDE the shape bounds. Total visible size equals
## `effective_size`; the fill is inset by `border_width` on each side.
...
func get_polygon_vertices(effective_size: Vector2) -> PackedVector2Array:
	...
	match shape_type:
		...
		ShapeType.TRIANGLE:
			return _make_regular_polygon(half, 3, -PI * 0.5)
		ShapeType.PENTAGON:
			return _make_regular_polygon(half, 5, -PI * 0.5)
		ShapeType.HEXAGON:
			return _make_regular_polygon(half, 6, -PI * 0.5)

	return PackedVector2Array()

## Vertices used for the FILL. When a border is active, the fill is inset
## by `border_width` on each side so the border's inner edge lines up with
## the fill boundary — no gap between them.
...
## Vertices used for the BORDER's outer edge — this is the shape's
## full bounding outline at `effective_size`.
